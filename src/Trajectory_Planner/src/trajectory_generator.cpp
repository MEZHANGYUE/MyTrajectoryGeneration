        ...
        const Eigen::VectorXd &Time) 
{
    ...
    const int M_block_rows = order * 2;    //行数
    const int M_block_cols = p_num1d;     //列数
    //M：转换矩阵，将系数向量转换为方程的微分量（A矩阵）
    ...
    {
        if (i < order)    //snap   ;  order = 4;   前4个
        {
            C_T(i, i) = 1;
            continue;
        }

        if (i >= number_coefficients - order)    //后4个
        {
        ...
        if ((i % order == 0) && (i / order % 2 == 1))   /// i 能4整除，并且是4的奇数倍；   i = 4 / 12 / 20 .......      ---->对应中间坐标点的末端位置信息
        {
        ...
        if ((i % order == 0) && (i / order % 2 == 0))     /// i是4的偶数倍；   i = 8 / 16 / 24 .......      ---->对应中间坐标点的初端位置信息
        {
    ...
    for (int k = 0; k < number_segments; ++k) 
    {
        ...
        for (int i = 0; i <= p_order; ++i) 
        {
            ...
            {
                if (p_num1d - i <= order || p_num1d - l <= order)     //Q矩阵 最后order 行和最后 order 列 都为 0 ；
                    continue;
